//Calculating the power & new temperature & new irradiance.

#ifndef NEWTEMPIRRA_H
#define NEWTEMPIRRA_H
...
void generateGraph2(const vector<double>& volt, const vector<double>& power, const vector<double>& volt2, const vector<double>& power2) {
    ...
    if (outputFile.is_open()) {
        ...
        for (int i = 0; i < volt.size(); i++) {
            outputFile << volt[i] << " " << power[i] << " " << volt2[i] << " " << power2[i] << endl;
        }
        outputFile.close();
    } 
    ...
    if (gnuplotScript.is_open()) {
        ...
        gnuplotScript << "set output 'TempIrrgraph.png'\n";
        ...
        gnuplotScript << "plot '-' with lines title 'temperature/irradiance 1', '-' with lines title 'temperature/irradiance 2'\n";
        ...
        for (int i = 0; i < volt2.size(); i++) {
            gnuplotScript << volt2[i] << " " << power2[i] << "\n";
        }
...
tuple<vector<double>, vector<double>, vector<double>,vector<double>, vector<double>, vector<double>> SetIrraTemp(){
    ...
    double irra = 0, temp = 0, irra2=0,temp2 = 0, I0 = 0, Iph = 0, I = 0, I_func = 0, I_prime = 0, errorval = 0, I_0 = 0, expo = 0, V_max=0;
    double new_V0=0, new_errorval=0 , new_V=0,new_V_func=0,new_V_prime=0;
    ...
    for( int i = 0; i <= 100; i++){
		try{
			...
			//cout<<"I max is "<<I<<endl;
			//cout<<"V max is "<<V_max<<endl;
		}
		catch (...){
			break;
		}
    ...
    // Find the volt current and power for the new temperature and irradiance the user entered 
	...
	cout << "Enter the new temperature" << endl;
    cin >> temp2;
	cout<<endl;
    ...
    I0 = (Isc + (a*(temp2-Tr)) ) / ( ( exp( (q*(Voc+(b*(temp2-Tr)))) / (n * k * temp2 * Ns ) ) ) -1 );

    Iph =(Isc + ( a * (temp2 - Tr)) )* (irra2/1000);



	//Calculate the voltage for the new values of irradiance and temperature
	
	new_V0 = Voc;
	
	do {
        
        new_V_func = Iph - I0 * ( ( exp((q * new_V0) / (n * k * temp2 * Ns) ) )- 1 )  - (new_V0 / Rsh);

        new_V_prime = ( (-I0) * ( q / (n * k * temp2 * Ns) ) * ( exp( ( q * new_V0 ) / (n * k * temp2 * Ns) ) ) ) - (1/Rsh);
	...
	cout << "V max is: " << new_V << endl;
	
    for (int i = 0; i <= 100; i++){
		double new_V0 = (i) * (new_V / 100);
       	volt2.push_back(new_V0);
	}

	//Calculate the current for the new values of irradiance and temperature
    ...
    for( int i = 0; i <= 100; i++){
		try{
			...
			do{
				...
				I_func= I_0 - Iph + (I0 * (exp ( q *( volt2[i]+ (I_0 *(Rs)))/(n*k*temp2*Ns))-1)) + ((volt2[i] +(I_0*(Rs) ))/Rsh);

				I_prime= 1 +  (I0*((( q *Rs)/(n*k*temp2*Ns)) * (exp ((q*( volt2[i]+ (I_0 *(Rs))))/(n*k*temp2*Ns)))))+ (Rs/Rsh);
	...
	//Calculate the power for the new values of irradiance and temperature
    ...
    cout << endl;
    cout << "max power when the temperature= "<<temp2<<" and the irradiance= "<<irra2 <<" is: "<< max_power2 << endl;
	cout<<endl;
  ...
  generateGraph2(volt, power, volt2, power2);
