//Calculating the power and the new temperature.
...
void generateGraph(const vector<double>& volt, const vector<double>& power, const vector<double>& volt2, 
	const vector<double>& power2 , const vector<double>& current , const vector<double>& current2 , 
    ...
    if (gnuplotScript.is_open()) {
        ...
        gnuplotScript << "set output 'Tempgraph.png'\n";
        gnuplotScript << "set title 'I-V curve for a constant irradiance'\n";
        gnuplotScript << "set xlabel 'Voltage (V)'\n";
        gnuplotScript << "set ylabel 'Current (I)'\n";
		gnuplotScript << "set key bottom left\n";
		
		temp=temp-273.15;
		temp2=temp2-273.15;
		
		gnuplotScript << "plot '-' with lines title 'temperature= " << temp << "°C', '-' with lines title 'temperature= " << temp2 << "°C'\n";
...
tuple<vector<double>, vector<double>, vector<double>,vector<double>, vector<double>,
    ...
    double irra = 0, temp = 0, temp2=0, I0 = 0, Iph = 0, I = 0, I_func = 0, I_prime = 0, errorval = 0,
		I_0 = 0, expo = 0,V0=0, V_max=0, new_V0=0, new_errorval=0 , new_V=0,new_V_func=0,
		new_V_prime=0, new_I_func=0 ,new_I_prime=0,new_I=0,new_I_0=0;
	

    vector<double> volt, current, power, volt2, current2, power2;

    tie(V_max, irra, temp) = MaxVolatge(); // here it will ask the user for time and fetch irra and temp
	...
	//Find the maximum power for the first time before changing any of the characteristics
	...
	// Find the volt current and power for the new temperature the user entered 
	
	
	cout << "Enter the value of the new temperature" << endl;
    cin >> temp2;
	
	cout<<endl;
	temp2 = temp2 +273.15;

	//Calculate I0 and Iph for the new temperature
    I0 = (Isc + (a*(temp2-Tr)) ) / ( ( exp( (q*(Voc+(b*(temp2-Tr)))) / (n * k * temp2 * Ns ) ) ) -1 );

    Iph =(Isc + ( a * (temp2 - Tr)) )* (irra/1000);
    ...
    for( int i = 0; i <= 100; i++){
		try{
			...
			do{
				...
				new_I_func= new_I_0 - Iph + (I0 * (exp ( q *( volt2[i]+ (new_I_0 *(Rs)))/(n*k*temp2*Ns))-1)) + ((volt2[i] +(new_I_0*(Rs) ))/Rsh);

				new_I_prime= 1 +  (I0*((( q *Rs)/(n*k*temp2*Ns)) * (exp ((q*( volt2[i]+ (new_I_0 *(Rs))))/(n*k*temp2*Ns)))))+ (Rs/Rsh);

				new_I = new_I_0 - (new_I_func / new_I_prime);
				
				errorval = abs(new_I - new_I_0);

				new_I_0 = new_I;

				if(errorval < error){
					current2.push_back(new_I);
				}		
    ...
    cout << endl;
    cout << "max power when the temperature= "<<temp2<<" and the irradiance= "<<irra <<" is: "<< max_power2 << endl;
	cout<<endl;
  ...
  generateGraph(volt, power, volt2, power2,current,current2,temp,temp2);
